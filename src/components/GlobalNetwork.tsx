...
type City = {
  ...
  tier: "primary" | "secondary" | "hub"; tz: string;
};
...
const ROUTES: [string, string][] = [
  ["India","Seattle"], ["India","Utah"], ["India","New York"],
  ["India","United Kingdom"], ["India","Canada"],
  ["Seattle","Utah"], ["Seattle","New York"], ["Seattle","Canada"],
  ["Seattle","United Kingdom"], ["Utah","New York"], ["Utah","Canada"],
  ["New York","Canada"], ["New York","United Kingdom"], ["Canada","United Kingdom"],
];

const GHOST_ROUTES: [string, string, number, boolean][] = [
  ["India","Seattle",         0.80, false],
  ...
  ["Seattle","United Kingdom",0.70, false],
...
const CONTINENT_POLYS: [number, number][][] = [
  // North America
  [[72,-141],[72,-68],[58,-62],[50,-55],[44,-66],[42,-70],[36,-76],
   [28,-80],[24,-81],[20,-87],[15,-83],[8,-77],[9,-79],[9,-83],
   [15,-88],[16,-90],[18,-100],[22,-106],[30,-115],[32,-117],
   [38,-122],[48,-124],[58,-136],[60,-163],[64,-168],[70,-163],
   [72,-152],[72,-141]],
  // South America
  [[12,-72],[10,-63],[8,-60],[4,-52],[2,-50],[-4,-36],[-10,-37],
   [-23,-43],[-30,-50],[-34,-53],[-40,-62],[-52,-68],[-55,-67],
   [-50,-73],[-42,-74],[-28,-71],[-18,-70],[-5,-80],[0,-80],
   [8,-77],[12,-72]],
  // Greenland
  [[76,-70],[76,-22],[70,-22],[60,-44],[60,-48],[62,-50],
   [64,-52],[68,-54],[72,-56],[76,-70]],
  // Eurasia (Europe + Russia + mainland Asia)
  [[72,-22],[72,28],[72,70],[72,142],[72,180],[60,162],
   [50,142],[42,132],[36,122],[22,114],[10,104],[1,104],
   [1,100],[6,100],[14,100],[18,94],[22,92],[20,88],
   [8,78],[8,76],[10,76],[18,74],[22,68],[24,66],
   [30,62],[38,52],[36,36],[30,34],[22,38],[12,44],
   [30,34],[36,36],[37,37],[37,10],[44,28],[50,20],
   [55,22],[60,28],[70,28],[72,-22]],
  // Africa
  [[37,10],[37,37],[30,34],[22,38],[12,44],[4,44],[0,42],
   [-10,40],[-20,35],[-26,33],[-35,26],[-34,18],[-30,16],
   [-18,12],[-4,12],[2,10],[8,4],[16,-17],[20,-17],
   [30,-10],[37,10]],
  // Australia
  [[-15,130],[-12,136],[-16,140],[-20,148],[-28,153],
   [-38,146],[-38,140],[-32,116],[-22,114],[-16,122],
   [-14,127],[-15,130]],
  // Japan (simplified)
  [[40,140],[35,135],[33,130],[34,131],[36,136],[38,141],[40,140]],
  // Great Britain
  [[58,-5],[60,-2],[58,0],[52,2],[50,-1],[50,-5],[52,-5],[54,-4],[58,-5]],
  // Iceland
  [[66,-24],[66,-13],[63,-14],[62,-20],[64,-24],[66,-24]],
  // New Zealand (South)
  [[-40,172],[-46,168],[-46,171],[-43,173],[-40,172]],
  // Madagascar
  [[-12,49],[-26,44],[-26,47],[-15,50],[-12,49]],
  // Ireland
  [[55,-6],[52,-10],[52,-6],[54,-6],[55,-6]],
];
...
function useClock(tz: string) {
  ...
  useEffect(() => {
    ...
    _clockSubs.add(tick);
    _startClock();
    return () => { _clockSubs.delete(tick); _stopClock(); };
  }, [tz]);
  return time;
}
...
function GlobeStage() {
  ...
  useEffect(() => {
    packetsRef.current = ROUTES.flatMap((_, route) =>
      [0, 0.34, 0.67].map((offset, i) => ({
        route, p: (Math.random() * 0.28 + offset) % 1,
        speed: 0.00080 + Math.random() * 0.00110,
        opacity: i === 0 ? 1 : 0.60, r: i === 0 ? 2.0 : 1.3,
      }))
    );
    ghostPacketsRef.current = GHOST_ROUTES.map((_, route) => ({
      route, p: Math.random(),
      speed: 0.00050 + Math.random() * 0.00060,
      opacity: 0.40, r: 1.2,
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const sphere = ctx.createRadialGradient(cx - radius*0.3, cy - radius*0.35, 0, cx, cy, radius);
      ...
      ctx.clip();
      const rim = ctx.createLinearGradient(cx - radius, cy - radius, cx + radius * 0.6, cy + radius * 0.6);
      ...
      for (const pt of spherePointsRef.current) {
        ...
        if (pt.land) {
          ...
          ctx.beginPath();
          ctx.arc(cx + v.x*radius, cy - v.y*radius, 0.55 + depth*0.90, 0, Math.PI*2);
          ctx.fill();
        } else {
      ...
      const spec = ctx.createRadialGradient(
        cx - radius*0.42, cy - radius*0.48, 0,
        cx - radius*0.42, cy - radius*0.48, radius*0.50,
      );
      ...
      // Advance packets
      for (const pkt of packetsRef.current)      { pkt.p += pkt.speed * dt; if (pkt.p >= 1) pkt.p = 0; }
      for (const pkt of ghostPacketsRef.current) { pkt.p += pkt.speed * dt; if (pkt.p >= 1) pkt.p = 0; }
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      lastPosRef.current = { x: e.clientX, y: e.clientY };
      rotYRef.current += dx * 0.4; rotXRef.current += dy * 0.4;
      velYRef.current = dx * 0.4 * 0.18; velXRef.current = dy * 0.4 * 0.18;
      lastInteractRef.current = performance.now();
    };
  ...
  const onQuad = (
    a: { x:number; y:number }, c: { x:number; y:number },
  ...
  return (
    <div ref={wrapRef} className="relative mx-auto aspect-square w-full select-none overflow-hidden rounded-full" style={{ maxWidth: 832 }}>
      <canvas ref={canvasRef} className="absolute inset-0 m-auto" style={{ willChange: "transform" }} />
      ...
      <svg width={size} height={size} className="absolute inset-0 m-auto pointer-events-none">
        ...
        {GHOST_ROUTES.map(([fr, to, lf, rev], i) => {
          const a = byName[fr], b = byName[to];
          if (!a || !b || (a.z+b.z)/2 < -0.18) return null;
          const c  = arcCP(a, b, lf, rev);
          const op = Math.max(0.04, Math.min(0.14, ((a.z+b.z)/2 + 0.4) * 0.20));
          return (
            <path key={`g${i}`}
              d={`M ${a.x} ${a.y} Q ${c.x} ${c.y} ${b.x} ${b.y}`}
              fill="none" stroke="url(#ghost-grad)" strokeWidth="0.65" opacity={op} />
          );
        })}
        ...
        {ROUTES.map(([fr, to], i) => {
          ...
          const op  = Math.max(0.20, Math.min(0.95, ((a.z+b.z)/2 + 0.42) * 1.1));
          const hub = fr === "India" || to === "India";
          return (
            <path key={`r${i}`}
              d={`M ${a.x} ${a.y} Q ${c.x} ${c.y} ${b.x} ${b.y}`}
              fill="none" stroke="url(#arc-grad)"
              strokeWidth={hub ? 1.6 : 0.80} opacity={op}
        ...
        {ghostPacketsRef.current.map((pkt, i) => {
          ...
          return <circle key={`gp${i}`} cx={pos.x} cy={pos.y} r={4.5} fill="url(#pkt-grad)" opacity={0.30} />;
        })}
        ...
        {packetsRef.current.map((pkt, i) => {
          ...
          const a = byName[fr], b = byName[to];
          if (!a || !b || (a.z+b.z)/2 < -0.10) return null;
          const pos = onQuad(a, arcCP(a,b,0.46), b, pkt.p);
          return (
            <g key={`pk${i}`}>
              <circle cx={pos.x} cy={pos.y} r={6}     fill="url(#pkt-grad)" opacity={pkt.opacity*0.55} />
        ...
        {projected.map((pt) => {
          ...
          const lop     = Math.max(0, Math.min(1, (pt.z + 0.10) * 2));
          const lo      = hub ? 18 : 14;
          ...
          return (
            <g key={pt.city.name}>
              ...
              <circle cx={pt.x} cy={pt.y} r={base} fill="var(--primary)" fillOpacity="0.10">
                <animate attributeName="r"       values={`${base*0.5};${base*2.3};${base*0.5}`} dur={hub?"3.5s":"5.2s"} repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.55;0;0.55"                            dur={hub?"3.5s":"5.2s"} repeatCount="indefinite"/>
              </circle>

              {/* Hub: inner ring + ambient circles */}
              {hub && (
                <>
                  <circle cx={pt.x} cy={pt.y} r={base*0.6} fill="var(--primary)" fillOpacity="0.22" filter="url(#hub-glow)">
                    <animate attributeName="r"       values={`${base*0.3};${base*1.1};${base*0.3}`} dur="2.1s" repeatCount="indefinite"/>
                    <animate attributeName="opacity" values="0.5;0;0.5"                              dur="2.1s" repeatCount="indefinite"/>
                  </circle>
                  <circle cx={pt.x} cy={pt.y} r={base*1.9} fill="none" stroke="var(--primary)" strokeWidth="0.55" opacity="0.18"/>
                  <circle cx={pt.x} cy={pt.y} r={base*3.0} fill="none" stroke="var(--primary)" strokeWidth="0.30" opacity="0.09"/>
                </>
              )}

              {/* Node dot */}
              <circle
                cx={pt.x} cy={pt.y} r={coreR}
                fill={hub ? "#fff" : "var(--primary)"}
                stroke="var(--primary)" strokeWidth={hub ? 2.2 : 0} strokeOpacity={hub ? 0.60 : 0}
              ...
              {lop > 0.05 && (
                <g opacity={lop}>
                  <line x1={pt.x} y1={pt.y} x2={pt.x+lo} y2={pt.y-lo}
                    stroke="var(--primary)" strokeOpacity="0.36" strokeWidth="0.6"/>
                  <rect
                    x={pt.x+lo} y={pt.y-(lo+20)}
                    width={(pt.city.name.length + (hub ? 4 : 0)) * 7.9 + 14}
                    height="20" rx="6"
                    fill="rgba(8,8,16,0.86)"
                    stroke="var(--primary)" strokeOpacity="0.26" strokeWidth="0.65"
                  />
                  <text
                    x={pt.x+lo+7} y={pt.y-lo-4}
...
export function GlobalNetwork() {
  return (
    ...
    <section className="globe-dark-section relative overflow-hidden px-6 py-28 lg:py-32">
      ...
      <div className="absolute left-1/2 top-1/3 h-[54rem] w-[54rem] -translate-x-1/2 rounded-full bg-primary/14 blur-[180px] pointer-events-none"/>
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2">
          {CLOCKS.map((c) => <ClockChip key={c.label} c={c}/>)}
        </div>
        ...
        <div className="mt-12 grid items-center gap-10 lg:grid-cols-[2.2fr_0.9fr]">
          ...
          <div className="flex flex-col gap-4">
            {STATUS_CARDS.map((s, i) => <StatusCard key={s.city} data={s} idx={i}/>)}
            <ActivityTicker/>
          </div>
        </div>
        ...
        <div className="mt-20 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {METRICS.map((m, i) => (
            <Reveal key={m.l} delay={i*0.08}>
              <div className="premium-card rounded-2xl p-6 text-center transition hover:bg-white/[0.06]">
